// KrakenD Pipeline: build → test → push ECR → deploy (local)
...
pipeline {
    ...
    parameters {
        ...
        booleanParam(name: 'RUN_TEST', defaultValue: true, description: 'Run config validation')
    ...
    stages {
        stage('Build') {
            steps {
                sh 'echo "Building KrakenD config for ${ENVIRONMENT}"'
            }
        }
            ...
            steps {
                sh """
                    docker run --rm -v \$(pwd):/etc/krakend \
                        devopsfaith/krakend check -c krakend.json --lint || true
                """
            }
        }
