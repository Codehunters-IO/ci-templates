// React Pipeline: install → build → test → push ECR → deploy (local)
...
pipeline {
    ...
    parameters {
        ...
        string(name: 'APP_PORT', defaultValue: '3000', description: 'External app port')
        string(name: 'DOCKER_PLATFORM', defaultValue: 'linux/arm64', description: 'Container platform')
        string(name: 'MEMORY_LIMIT', defaultValue: '256m', description: 'Container memory limit')
        string(name: 'MEMORY_RESERVATION', defaultValue: '128m', description: 'Container memory reservation')
